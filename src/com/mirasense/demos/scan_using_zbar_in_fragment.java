...
import com.openerp.addons.idea.menu_add_asset;
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            if (result != 0) {
                ...
                for (Symbol sym : syms) 
                {
                    ...
                    if(OEHelper.qr_equip_asset_bar_code.size()!=0 )
            		{
            			if(OEHelper.qr_equip_asset_bar_code.size()>QR_Equipment.position1 )
            			{
            				if(OEHelper.qr_equip_asset_bar_code.get(QR_Equipment.position1).equals(sym.getData()))
            				{
            					...
            					QR_equip_detail QR=new QR_equip_detail();
                    			FragmentListener frag = (FragmentListener) getActivity();
                    			frag.startDetailFragment(QR);
            				}
            				
            				else
            				{
            					
            					//Toast.makeText(getActivity(), "BAR Code Not Matched ", 30).show();
            					
            					Log.i("barcode is..."+sym,"THIS IS BAR CODE...");
            					
            					Toast.makeText(getActivity(), "Bar Code is: = "+sym, 90).show();
            				}
